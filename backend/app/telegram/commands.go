...
func setupCommandHandlers() {
	AllCommandHandlers = []*CommandHandler{
		...
		newCommandHandler(TermsCommand, func(ctx context.Context, bot *Bot, message *telego.Message) {
			...
			bot.SendMessage(context.Background(), tu.Message(util.GetChatID(message), USAGE_TERMS_URL).WithMessageThreadID(message.MessageThreadID))
		}),
		...
		newCommandHandler(SYSTEMSetOnboardingVideoCommand, func(ctx context.Context, bot *Bot, message *telego.Message) {
			...
			if message.Video == nil {
				bot.SendMessage(context.Background(), tu.Message(SystemBOT.ChatID, "Please provide video").WithMessageThreadID(message.MessageThreadID))
				return
			}
			err := redis.RedisClient.Set(ctx, "onboarding-video", message.Video.FileID, 0).Err()
			if err != nil {
				bot.SendMessage(context.Background(), tu.Message(SystemBOT.ChatID, fmt.Sprintf("Failed to save video: %v", err)).WithMessageThreadID(message.MessageThreadID))
				return
			}
			bot.SendMessage(context.Background(), tu.Message(SystemBOT.ChatID, "Onboarding video saved").WithMessageThreadID(message.MessageThreadID))
		}),
		...
		newCommandHandler(BillingCommand, func(ctx context.Context, bot *Bot, message *telego.Message) {
			...
			if user.StripeCustomerId == "" {
				bot.SendMessage(context.Background(), tu.Message(chatID, "You don't have billing setup.").WithMessageThreadID(message.MessageThreadID))
				return
			}
...
func (c CommandHandlers) handleCommand(ctx context.Context, bot *Bot, message *telego.Message) {
	...
	if commandHandler != nil {
		...
		bot.SendMessage(context.Background(), tu.Message(util.GetChatID(message), "Unknown command \U0001f937").WithMessageThreadID(message.MessageThreadID))
	}
}
...
func getModeHandlerFunction(mode lib.ModeName, response string) func(context.Context, *Bot, *telego.Message) {
	return func(ctx context.Context, bot *Bot, message *telego.Message) {
		...
		bot.SendMessage(context.Background(), tu.Message(util.GetChatID(message), response).WithMessageThreadID(message.MessageThreadID))
...
func upgradeCommandHandler(ctx context.Context, bot *Bot, message *telego.Message) {
	...
	if lib.IsUserBasic(ctx) {
		bot.SendMessage(context.Background(), tu.Message(chatID, "You are already a basic paid plan user! Premium upgrade plans are not available yet. Stay tuned for updates!").WithMessageThreadID(message.MessageThreadID))
		return
	}
...
func cancelSubscriptionCommandHandler(ctx context.Context, bot *Bot, message *telego.Message) {
	...
	if lib.IsUserFree(ctx) {
		bot.SendMessage(context.Background(), tu.Message(chatID, "You are already a free user!").WithMessageThreadID(message.MessageThreadID))
		return
	}
	...
	bot.SendMessage(context.Background(), tu.Message(chatID, confirmationMessage).WithMessageThreadID(message.MessageThreadID).WithReplyMarkup(
		&telego.InlineKeyboardMarkup{
			InlineKeyboard: [][]telego.InlineKeyboardButton{
				{
					telego.InlineKeyboardButton{
...
func emptyCommandHandler(ctx context.Context, bot *Bot, message *telego.Message) {
	_, err := bot.SendMessage(context.Background(), tu.Message(util.GetChatID(message), "There is no message provided to correct or comment on. If you have a message you would like me to review, please provide it.").WithMessageThreadID(message.MessageThreadID))
	if err != nil {
...
func supportCommandHandler(ctx context.Context, bot *Bot, message *telego.Message) {
	...
	_, err := bot.SendMessage(context.Background(), supportMessage)
	if err != nil {
...
func statusCommandHandler(ctx context.Context, bot *Bot, message *telego.Message) {
	_, err := bot.SendMessage(context.Background(), tu.Message(util.GetChatID(message), GetUserStatus(ctx)).WithMessageThreadID(message.MessageThreadID).WithReplyMarkup(GetStatusKeyboard(ctx)))
	if err != nil {
...
func clearThreadCommandHandler(ctx context.Context, bot *Bot, message *telego.Message) {
	...
	_, err := bot.SendMessage(context.Background(), tu.Message(chatID, "All memory cleared!").WithMessageThreadID(message.MessageThreadID))
	if err != nil {
...
func startCommandHandler(ctx context.Context, bot *Bot, message *telego.Message) {
	...
	chatId := util.GetChatID(message)
	_, err := bot.SendMessage(context.Background(), tu.Message(chatId, notification).WithMessageThreadID(message.MessageThreadID).WithReplyMarkup(GetStatusKeyboard(ctx)))
	if err != nil {
...
func sendGeneralOnboardingVideo(ctx context.Context, bot *Bot, message *telego.Message) {
	...
	_, err := bot.SendVideo(context.Background(), &telego.SendVideoParams{
		ChatID: util.GetChatID(message),
